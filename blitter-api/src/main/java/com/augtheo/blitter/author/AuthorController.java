...
public class AuthorController implements UsersApi , RegisterApi {
	...
	@Autowired
	AuthorController(AuthorService authorService, FavouriteService favouriteService,
			BleatRepository bleatRepository , PasswordEncoder passwordEncoder) {
	...
	public ResponseEntity<List<BleatRes>> usersUsernameBleatsGet(String username) {
		return 	ResponseEntity.ok(
				bleatRepository.findAllByAuthor(authorService.getAuthorByUsername(username)).stream().map(this::domainModelConverter).toList()
		);
	}
	...
	public ResponseEntity<List<BleatRes>> usersUsernameLikedGet(String username) {
		return ResponseEntity.ok(
				favouriteService.getLikedBleats(authorService.getAuthorByUsername(username)).stream().map(this::domainModelConverter).toList()
		);
	}
